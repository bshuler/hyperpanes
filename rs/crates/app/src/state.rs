...
impl State {
    ...
    ///
    /// **M7: claim first, adopt second.** The claim is a compare-and-set in the daemon's
    /// registry, so if two windows (in two processes) click adopt on the same orphan at the
    /// same moment, exactly one of them is granted it and the other returns here having
    /// changed nothing. Losing is silent by design: the winner's claim reaches this process
    /// on the next pushed snapshot and the row simply leaves the DETACHED list.
